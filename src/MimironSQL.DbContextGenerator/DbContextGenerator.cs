...
public sealed class DbContextGenerator : IIncrementalGenerator
{
    ...
    private static string RenderContext(ImmutableArray<EntitySpec> entities)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("public partial class WoWDb2Context : DbContext");
        sb.AppendLine("{");
        sb.AppendLine("    public WoWDb2Context(DbContextOptions<WoWDb2Context> options)");
        ...
        {
            sb.AppendLine($"    public DbSet<{e.ClassName}> {e.ClassName} {{ get; set; }} = null!;");
        }
        ...
        foreach (var e in entities.OrderBy(e => e.ClassName, StringComparer.Ordinal))
        {
            RenderEntityConfiguration(sb, e, entities);
        }
    ...
    private static void RenderEntityConfiguration(StringBuilder sb, EntitySpec entity, ImmutableArray<EntitySpec> allEntities)
    {
        var byTableName = allEntities.ToDictionary(e => e.TableName, e => e, StringComparer.Ordinal);

        sb.AppendLine($"        modelBuilder.Entity<{entity.ClassName}>(entity =>");
        sb.AppendLine("        {");

        if (!string.Equals(entity.TableName, entity.ClassName, StringComparison.Ordinal))
        {
            sb.AppendLine($"            entity.ToTable(\"{EscapeString(entity.TableName)}\");");
        }

        var idProperty = entity.ScalarProperties.FirstOrDefault();
        ...
        foreach (var p in entity.ScalarProperties)
        {
            ...
            {
                var cleanPropertyName = p.PropertyName.TrimStart('@');
                sb.AppendLine($"            entity.Property(e => e.{cleanPropertyName})");
                sb.AppendLine($"                .HasColumnName(\"{EscapeString(p.ColumnName)}\");");
            }
        }
        ...
        foreach (var p in entity.ScalarProperties)
        {
            ...
            foreach (var nav in navs)
            {
                ...
                var cleanPropertyName = p.PropertyName.TrimStart('@');
                var cleanNavPropertyName = nav.PropertyName.TrimStart('@');

                if (nav.IsCollection)
                {
                    sb.AppendLine($"            entity.HasMany(e => e.{cleanNavPropertyName})");
                    sb.AppendLine($"                .WithOne()");
                    sb.AppendLine($"                .HasForeignKey(\"{cleanPropertyName}\");");
                }
                else
                {
                    sb.AppendLine($"            entity.HasOne(e => e.{cleanNavPropertyName})");
                    sb.AppendLine($"                .WithMany()");
                    sb.AppendLine($"                .HasForeignKey(e => e.{cleanPropertyName});");
                }
            }
        }
