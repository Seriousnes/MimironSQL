...
public sealed class DbContextGenerator : IIncrementalGenerator
{
    ...
    private static void RenderEntityConfiguration(StringBuilder sb, EntitySpec entity, ImmutableArray<EntitySpec> allEntities)
    {
        ...
        foreach (var p in entity.ScalarProperties.Where(p => p.ColumnName is not null))
        {
            sb.AppendLine($"            entity.Property(e => e.{p.PropertyName})");
        ...
        foreach (var p in entity.ScalarProperties.Where(p => navsByForeignKey.ContainsKey(p.PropertyName)))
        {
            var navs = navsByForeignKey[p.PropertyName];

            foreach (var nav in navs.Where(n => !n.IsCollection))
            {
                sb.AppendLine($"            entity.HasOne(e => e.{nav.PropertyName})");
                sb.AppendLine($"                .WithMany()");
                sb.AppendLine($"                .HasForeignKey(e => e.{p.PropertyName});");
            }
        }
