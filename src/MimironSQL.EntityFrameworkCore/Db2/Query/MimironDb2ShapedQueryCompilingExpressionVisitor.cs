    ...
    : ShapedQueryCompilingExpressionVisitor(dependencies, queryCompilationContext)
{
    ...
    private static IEnumerable<Expression> SplitAndAlso(Expression expression)
    {
        while (expression is UnaryExpression { NodeType: ExpressionType.Convert or ExpressionType.ConvertChecked } u)
            expression = u.Operand;
    ...
    private static Func<QueryContext, IDb2File, RowHandle, bool>? TryCompileRowHandlePredicate(
    ...
    private static bool TryTranslateConjunct(
        IEntityType entityType,
    ...
    private static bool TryTranslateFieldAccess(
        IEntityType entityType,
        StoreObjectIdentifier storeObject,
        Db2TableSchema schema,
        ParameterExpression entityParameter,
    ...
    private static bool TryGetVirtualIdPrimaryKeyEqualityLookup(
        IEntityType entityType,
        string tableName,
        Db2TableSchema schema,
        IReadOnlyList<LambdaExpression> predicates,
        ...
        out Type keyType)
    {
        ...
        var fieldSide = matchedLeft ? left : right;
    ...
    private static IEnumerable<ValueBuffer> Table(QueryContext queryContext, Db2QueryExpression queryExpression)
    {
        ...
        if (queryExpression.Joins.Count == 0)
        {
            ...
            if (TryGetVirtualIdPrimaryKeyEqualityLookup(entityType, tableName, schema, queryExpression.Predicates, out var getKeyValue, out var keyType))
            {
            ...
            var rowPredicate = TryCompileRowHandlePredicate(entityType, tableName, schema, queryExpression.Predicates);
        ...
        var rootRowPredicate = TryCompileRowHandlePredicate(entityType, tableName, rootSchemaForJoin, queryExpression.Predicates);
